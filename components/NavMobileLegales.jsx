          ...
          <div className="flex flex-col gap-y-12">
            {links.map((link, index) => {
              return link.customUrl ? (
                <Link className= 'uppercase text-sm text-botons text-end' target="black" href={link.customUrl} key={index}>
                  {link.name}
                </Link>
              ) : (
                <ScrollLink
                  ...
                  offset={link.offset}
                  smooth={false}
                  className="text-end uppercase text-sm text-botons"
                  onClick={() => setIsOpen(false)}
                >
